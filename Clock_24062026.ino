...
struct Config {
  ...
  int scrollSpeed;         // 10 - 100
  int displayIntensity;    // 0 - 15
  bool use24Hour;
  bool showDate;
  bool showDay;
  bool enableWeather;
...
void startScrollingMessage(String msg) {
  ...
  P->displayText(displayMessage, PA_CENTER, config.scrollSpeed, pauseTime, effect, effect);
  P->displayReset();
}
...
void scrollIPBlocking() {
  ...
  P->displayText(displayMessage, PA_CENTER, config.scrollSpeed, 0, PA_SCROLL_LEFT, PA_SCROLL_LEFT);
  P->displayReset();
...
void loadConfiguration() {
  ...
  if (!error) {
    config.numModules = doc["numModules"] | 4;
    config.scrollSpeed = doc["scrollSpeed"] | 45;
    config.displayIntensity = doc["displayIntensity"] | 2;
...
void handleRootPage() {
  ...
  html += "Speed Vector Delay (10-100): <input type='number' name='speed' value='" + String(config.scrollSpeed) + "'><br>";
  html += "Brightness Level (0-15): <input type='number' name='intensity' value='" + String(config.displayIntensity) + "'><br>";
...
void setup() {
  ...
  // CHANGED: Show a clean, solid "WIFI" status text immediately at boot
  ...
  if (WiFi.status() == WL_CONNECTED) {
    // FIXED: Clean scroll execution of the IP address right here on startup connection success
    scrollIPBlocking();
    ...
    portalActive = true;
    displayStaticText("AP"); // CHANGED: Show a solid, readable "AP" text waiting for phone connection
  }
...
void loop() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    // FIXED: If we have connected but haven't scrolled the IP address yet, do it now
    if (!ipScrollDone) {
      scrollIPBlocking();
