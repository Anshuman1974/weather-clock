 ...
 * ADD: Web-based Over-The-Air (OTA) HTTP Update capability
 */
...
#include <ESP8266httpUpdate.h> // OTA Update Library
...
void loadConfiguration() {
  ...
  if (!error) {
    config.numModules = doc["numModules"] | 8;
    config.scrollSpeed = doc["scrollSpeed"] | 60;
    config.displayIntensity = doc["displayIntensity"] | 80;
...
void handleRootPage() {
  ...
  html += F("<a href='/do-ota' class='btn btn-warn' style='background:#9C27B0;' onclick='return confirm(\"Download and install the latest firmware from the server? Make sure power is stable.\")'>Update Firmware</a>");
  html += F("</div>");
...
void handleOTAUpdate() {
  // Send a response to the browser immediately so it doesn't hang
  server.send(200, "text/html", F("<html><head><meta name='viewport' content='width=device-width, initial-scale=1.0'></head><body style='font-family:sans-serif; text-align:center; margin-top:50px;'><h3>Downloading update from GitHub...</h3><p>The clock display will freeze for a moment. If successful, it will reboot automatically.</p></body></html>"));
  delay(1000);

  // USE WIFICLIENTSECURE FOR GITHUB (HTTPS)
  WiFiClientSecure client;
  
  // By setting this to insecure, the ESP8266 will not check the SSL certificate fingerprint.
  // This is required because GitHub's certificates change, and a hardcoded fingerprint would eventually break OTA updates.
  client.setInsecure(); 
  ...
  if (P != nullptr) {
    P->displayClear();
    P->displayText("UPDATING", PA_CENTER, 0, 0, PA_PRINT, PA_PRINT);
    P->displayAnimate();
  }
  ...
  // If we reach this point, the update failed (a successful update reboots the device)
  if (ret == HTTP_UPDATE_FAILED) {
    Serial.printf("HTTP_UPDATE_FAILED Error (%d): %s\n", ESPhttpUpdate.getLastError(), ESPhttpUpdate.getLastErrorString().c_str());
    if (P != nullptr) {
      P->displayClear();
      P->displayText("FAILED", PA_CENTER, 0, 0, PA_PRINT, PA_PRINT);
      P->displayAnimate();
      delay(3000);
      ESP.restart(); // Reboot to clear memory and resume normal operation
    }
  }
}
...
void setup() {
  ...
  server.on("/do-ota", handleOTAUpdate); 
  server.begin();
}
